...
operations on this array should also use the 64-bit versions, if available. In
addition, pointers to internal arrays that access CSR formatted blocks of the
array should also be of type int64_t. It is also advisable to configure Global
...
\begin{capi}
\begin{ccode}
void NGA_Sprs_array_access_col_block(int s_a, int iproc, void *idx, void *jdx,
void *val)
\end{ccode}
\begin{funcargs}
\inarg{int}{s_a}{handle for sparse array}
\inarg{int}{iproc}{process rank associated with column block}
\outarg{void*}{idx}{pointer to offsets for each row}
\outarg{void*}{jdx}{pointer to column indices for each row}
...
\inarg{integer/real/complex}{value}{element value}
\end{fapi}

\ncoll

\begin{desc}
Add a non-zero element to array at the location (idx,jdx). Any
process can add any element but an element should only be added once to the
array.
\end{desc}
...
\begin{desc}
This operation distributes the individual matrix elements added to the array to
the appropriate processors and organizes the data into sub-blocks within each processor.
All add element operations should be completed before calling this function and
no additional elements can be added after calling this function.

After calling the assemble function, each processor has a row block of the
sparse array and each row block is further divided into sparse sub-blocks
corresponding to each processor. For most applications, the number of sub-blocks
with non-zero elements will be smaller than the number of processors.

This function returns true if the assembly is successful.
\end{desc}
...
\begin{capi}
\begin{ccode}
void NGA_Sprs_array_column_distribution(int s_a, int idim, int iproc, int *jlo, int *jhi)
\end{ccode}
\begin{funcargs}
\inarg{int}{s_a}{handle for sparse array}
...
\begin{desc}
This function returns the column indices associated with processor iproc. Note
that ownership of matrix elements is solely determined by the row index but
within a row block the data is partitioned into blocks associated with each
process. Some blocks may have no data. The low and high column indices of this
secondary partition are returned by this function. This may be used to obtain
pointers to these individual blocks.
\end{desc}

\apih{SPRS ARRAY CREATE}{Create a sparse matrix data structure}
...
\end{ccode}
\begin{funcargs}
\inarg{int}{idim}{row dimension of array}
\inarg{int}{jdim}{column dimension of array}
\inarg{int}{type}{array data type}
\end{funcargs}
\end{capi}
...
\inarg{integer}{type}{array data type}
\end{fapi}

\gcoll

\begin{desc}
This function returns a handle to a sparse array data structure corresponding to
an idim x jdim sized matrix of type type.
\end{desc}
...
\begin{verbatim}
          ...
          i(N), j(N), val(N)
\end{verbatim}
for N non-zero values. This function is designed to be a debugging tool and
should not be used for extremely large matrices.
\end{desc}
...
\apih{SPRS ARRAY LEFT DIAG MULTIPLY}{Left multiply the sparse matrix by a diagonal matrix}
...
\apih{SPRS ARRAY MATVEC MULTIPLY}{Multiplay a sparse matrix by a vector}
...
\apih{SPRS ARRAY RIGHT DIAG MULTIPLY}{Right multiply the sparse matrix by a diagonal matrix}
...
\apih{SPRS ARRAY ROW DISTRIBUTION}{What rows are owned by each process}

\begin{capi}
\begin{ccode}
void NGA_Sprs_array_row_distribution(int s_a, int idim, int iproc, int *ilo, int *ihi)
\end{ccode}
\begin{funcargs}
\inarg{int}{s_a}{handle for sparse array}
\inarg{int}{iproc}{process rank}
\outarg{int*}{ilo}{lowest row index owned by process iproc}
\outarg{int*}{ihi}{highest row index owned by process iproc}
\end{funcargs}
\end{capi}

\begin{fapi}
\begin{fcode}
subroutine nga_sprs_array_row_distribution(s_a, iproc, ilo, ihi)
\end{fcode}
\inarg{integer}{s_a}{handle for sparse array}
\inarg{integer}{iproc}{process rank}
\outarg{integer}{ilo}{lowest row index owned by process iproc}
\outarg{integer}{ihi}{highest row index owned by process iproc}
\end{fapi}

\ncoll

\begin{desc}
This function returns the row block owned by processor iproc, All rows between
ilo and ihi, inclusive, are stored on process iproc.
\end{desc}
...
\end{fcode}
\inarg{int}{s_a}{handle for sparse array}
\inarg{integer/real/complex}{val}{amount to shift diagonal}
\end{fapi}
...
\apih{VERSION}{Returns version information about the current build}
...
\end{ccode}
\begin{funcargs}
\outarg{int}{major}{major release version}
\outarg{int}{minor}{minor release version}
...
\begin{fapi}
\begin{fcode}
subroutine nga_version(integer major, integer minor, integer patch)
\end{fcode}
\begin{funcargs}
\outarg{integer}{major}{major release version}
\outarg{integer}{minor}{minor release version}
\outarg{integer}{patch}{patch/hotfix release number}
\end{funcargs}
\end{fapi}

\begin{desc}
This function can be used to query Global Arrays about which version
