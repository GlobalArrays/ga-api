...
\begin{pyapi}
\begin{pycode}
g_a = ga.create_irreg(int gtype, dims, width, block, map, char *name='',
                      int pgroup=-1)
\end{pycode}
\begin{funcargs}
\inarg{int}{gtype}{data type (C_DBL,C_INT,C_DCPL)}
...
\begin{pyapi}
\begin{pycode}
ret = access_ghosts(int g_a)
\end{pycode}
\begin{funcargs}
\inarg{int}{g_a}{the array handle}
\outarg{ndarray}{ret}{appropriately shaped, possibly non-contiguous ndarray
corresponding to global array patch held locally on the processor}
\end{funcargs}
\end{pyapi}
...
\inarg{void*}{buf}{pointer to local buffer array where data goes}
\inarg{int*}{ld[ndim-1]}{array specifying leading dimensions/strides/extents for buffer array}
\inarg{void*}{alpha}{double/DoublComplex/long scale factor}
\end{funcargs}
\end{cxxapi}
...
The addressing convention refers the first element \verb|(ilo,jlo)| of the
patch. However, you can only pass that reference to another subroutine where it
could be used like a normal array, see the following example. This constraint
caused by the HP fortran compiler inability to reference shared memory data
...
\seealso{ACCESS,ACCESS BLOCK,RELEASE BLOCK SEGMENT,RELEASE UPDATE BLOCK SEGMENT}
...
\begin{pyapi}
\begin{pycode}
ret = access_block_grid(int g_a, int idx)
\end{pycode}
\begin{funcargs}
\inarg{int}{g_a}{array handle}
\inarg{int}{idx}{block index}
...
\begin{pyapi}
\begin{pycode}
ret = access_block_grid(int g_a, subscript)
\end{pycode}
\begin{funcargs}
\inarg{int}{g_a}{array handle}
...
(v,subscrArray) are preserved, but their contents might be (consistently)
shuffled on return.
...
the input arrays (v, subscrArray) are preserved, but their contents might be
...
by alpha. The contents of the input arrays (v, subscrArray) are preserved, but
...
that represents error code. Releases some system resources. This is the
...
Return in owner the GA compute process ID that `owns' the data. If any element
