...
\apih{DEALLOCATE}{Deallocates memory for a global array without removing the array}
...
\apih{DISTRIBUTION}{Reports the data range held on a specified processor}
...
\seealso{SET BLOCK CYCLIC,SET BLOCK CYCLIC PROC GRID,SET TILED PROC GRID}
...
\apih{GET GHOST BLOCK}{Gets data, including ghost cells, from a global array with ghost cells}
...
\seealso{GET,ACCESS GHOSTS,RELEASE GHOSTS,RELEASE UPDATE GHOSTS}
...
\apih{INITIALIZE COMM}{Initializes GA on an MPI communicator}
...
\end{ccode}
\begin{funcargs}
\inarg{MPI_Comm}{comm}{MPI communicator used to initialize GA}
\end{funcargs}
\end{capi}

\begin{fapi}
\begin{fcode}
logical function nga_initialize_comm(comm)
logical function ga_initialize_comm(comm)
\end{fcode}
\begin{funcargs}
\inarg{MPI_Comm}{comm}{MPI communicator used to initialize GA}
\end{funcargs}
...
restricted set of processors described by comm. This can be used to initialize
...
This initialization returns a logical value that is used to guarantee that the
program does not hang when exiting the subroutine initializing GA. This can
happen when using the progress ranks runtime. A properly constructed function
using this initialization is shown below.
\end{desc}

\begin{fdesc}
\begin{verbatim}
subroutine my_ga_app(...., comm)
  MPI_Comm comm
  if (nga_initialize(comm)) then
...
\begin{desc}
For the progress rank runtime or other runtimes that use a helper process to
manage asynchronous progress, the conditional guarantees that the function or
subroutine exits cleanly. Without the conditional, the progress ranks will
attempt to execute the code after the application has finished, usually with
poor results.
\end{desc}

\seealso{INITIALIZE,INITIALIZE LTD}
...
\apih{NBTEST}{Tests a non-blocking GA operation for completion}
...
\outarg{int}{}{1 if operation has completed}
\end{funcargs}
\end{capi}
...
\outarg{logical}{}{.TRUE. if operation has completed}
\end{funcargs}
\end{fapi}
...
\seealso{NBWAIT,NBGET,NBPUT,NBACC}

\apih{NBWAIT}{Waits for a non-blocking GA operation to complete}
...
\seealso{NBTEST,NBGET,NBPUT,NBACC}
...
\apih{OVERLAY}{Create a global array on top of an existing allocation}
...
\end{ccode}
\begin{funcargs}
\inarg{int}{g_a}{handle for overlay array}
\inarg{int}{g_p}{handle for parent array}
\end{funcargs}
\end{capi}
...
\end{fcode}
\begin{funcargs}
\inarg{integer}{g_a}{handle for overlay array}
\outarg{integer}{g_p}{handle for parent array}
\end{funcargs}
\end{fapi}
...
\end{cxxcode}
\begin{funcargs}
\inarg{const GlobalArray*}{g_p}{parent array}
\end{funcargs}
\end{cxxapi}
...
\apih{PGROUP DUPLICATE}{Duplicate an existing GA processor group}
...
\begin{funcargs}
\inarg{int}{p_handle}{processor group handle}
\outarg{int}{}{handle to new processor group}
\end{funcargs}
\end{capi}
...
\begin{funcargs}
\inarg{integer}{p_handle}{processor group handle}
\outarg{integer}{}{handle to new processor group}
\end{funcargs}
\end{fapi}
...
\apih{PGROUP SELF}{Create a process group with only the calling process in it}
...
\apih{PGROUP SPLIT}{Creates a set of new groups by dividing a parent group into equal sized subgroups}

\begin{capi}
\begin{ccode}
int GA_Pgroup_split(int p_handle, int num_group)
int NGA_Pgroup_split(int p_handle, int num_group)
\end{ccode}
\begin{funcargs}
\inarg{int}{p_handle}{processor group handle of parent}
\inarg{int}{num_group}{number of new groups}
\end{funcargs}
\end{capi}

\begin{fapi}
\begin{fcode}
integer function ga_pgroup_split(p_handle, num_group)
\end{fcode}
\begin{funcargs}
\inarg{integer}{p_handle}{processor group handle}
\inarg{integer}{num_group}{number of new groups}
\end{funcargs}
\end{fapi}
...
This function can be used to divide the parent group, represented by p_handle,
into num_group smaller groups. The function returns the group handle of the subgroup
containing the calling processor. If num_group does not evenly divide the number
of processors in p_handle, then one group will contain fewer processors than the
others. All processors in the group referenced by p_handle must make a
...
\apih{PGROUP SPLIT IRREG}{Creates a set of new groups based on a color variable}

\begin{capi}
\begin{ccode}
int GA_Pgroup_split_irreg(int p_handle, int color)
int NGA_Pgroup_split_irreg(int p_handle, int color)
\end{ccode}
\begin{funcargs}
\inarg{int}{p_handle}{processor group handle of parent}
\inarg{int}{color}{color variable of calling process}
\end{funcargs}
\end{capi}

\begin{fapi}
\begin{fcode}
integer function ga_pgroup_split_irreg(p_handle, color)
\end{fcode}
\begin{funcargs}
\inarg{integer}{p_handle}{processor group handle}
\inarg{integer}{color}{color variable of calling process}
\end{funcargs}
\end{fapi}
...
This function can be used to divide the parent group, represented by p_handle,
into subgroups of different sizes based on the color variable. All processes
with the same color variable end up in the same group.  The function returns the
group handle of the subgroup containing the calling processor. Color variables
should be greater than or equal to zero.
All processors in the group referenced by p_handle must make a
call to this function.
...
\apih{RAND}{Random number generator}
...
\end{ccode}
\begin{funcargs}
\inarg{int}{iseed}{integer seed for random number generator}
\end{funcargs}
\end{capi}
...
\end{fcode}
\begin{funcargs}
\inarg{integer}{iseed}{integer seed for random number generator}
\end{funcargs}
\end{f2dapi}
...
from \href{https://github.com/ESultanik/mtwister}{here}.
...
\apih{SET MEMORY DEV}{Specify the type of memory used for the global array}
...
\begin{funcargs}
\inarg{int}{g_a}{global array handle}
\inarg{char*}{device}{character string specifying the memory type}
\end{funcargs}
\end{capi}
...
\begin{funcargs}
\inarg{integer}{g_a}{global array handle}
\inarg{integer}{character*(*)}{character string specifying the memory type}
\end{funcargs}
\end{fapi}
...
\end{cxxcode}
\begin{funcargs}
\inarg{char*}{property}{character string specifying the memory type}
\end{funcargs}
\end{cxxapi}
...
\apih{SET PROPERTY}{Assign a property to a global array}
...
\begin{funcargs}
\inarg{int}{g_a}{global array handle}
\inarg{char*}{property}{character string representing the property}
\end{funcargs}
\end{capi}
...
\begin{funcargs}
\inarg{integer}{g_a}{global array handle}
\inarg{integer}{character*(*)}{character string representing the property}
\end{funcargs}
\end{fapi}
...
\apih{SET TILED IRREG PROC GRID}{This function specifies that the GA will use a distribution of irregularly sized blocks that will be assigned to processors using SCALAPACK-type block cyclic data distribution with individual blocks being contiguous in memory}
...
\begin{funcargs}
\inarg{int}{g_a}{global array handle}
\inarg{int}{mapc[]}{starting index of each block}
\inarg{int}{nblocks[]}{array containing the number of blocks in each dimension}
\inarg{int}{proc_grid[]}{processor grid dimensions}
\end{funcargs}
\end{capi}
...
\begin{funcargs}
\inarg{integer}{g_a}{global array handle}
\inarg{integer}{mapc(s)}{starting index of each block. s is equal to the sum of indices in nblocks array}
\inarg{integer}{nblocks(ndim)}{array containing the number of blocks in each dimension}
\inarg{integer}{proc_grid(ndim)}{processor grid dimensions}
\end{funcargs}
\end{fapi}
...
\begin{funcargs}
\inarg{int}{mapc[]}{starting index of each block}
\inarg{int}{nblocks[]}{array containing number of blocks in each dimension}
\inarg{int}{proc_grid[]}{processor grid dimensions}
\end{funcargs}
\end{cxxapi}
...
\seealso{SET BLOCK CYCLIC PROC GRID,SET IRREG DISTR}

\apih{SET TILED PROC GRID}{This function specifies that the GA will use a regular SCALAPACK-type block cyclic data distribution but individual blocks will be contiguous in memory}
...
\begin{funcargs}
\inarg{int}{g_a}{global array handle}
\inarg{int*}{block_dims[]}{array of block dimensions}
\inarg{int*}{proc_grid[]}{processor grid dimensions}
\end{funcargs}
\end{capi}
...
\begin{funcargs}
\inarg{integer}{g_a}{global array handle}
\inarg{integer}{block_dims(ndim)}{array of block dimensions}
\inarg{integer}{proc_grid(ndim)}{array of processor grid dimensions}
\end{funcargs}
\end{fapi}
...
\apih{SPRS ARRAY ACCESS COL BLOCK}{Directly access a locally owned sublock of matrix}

\begin{capi}
\begin{ccode}
void NGA_Sprs_array_access_col_block(int s_a, int iproc, int **idx, int **jdx,
void **val)
void NGA_Sprs_array_access_col_block64_t(int s_a, int iproc, int64_t **idx,
int64_t **jdx,
void **val)
\end{ccode}
\begin{funcargs}
\inarg{int}{s_a}{handle for sparse array}
\inarg{int}{iproc}{process rank associated with column block}
\outarg{int**}{idx}{pointer to offsets for each row}
\outarg{int**}{jdx}{pointer to column indices for each row}
\outarg{void**}{val}{pointer to values for each row}
\end{funcargs}
\end{capi}

\begin{fapi}
\begin{fcode}
subroutine nga_sprs_array_access_col_block(s_a, iproc, idx, jdx, val)
\end{fcode}
\inarg{integer}{s_a}{handle for sparse array}
\inarg{integer}{iproc}{process rank associated with column block}
\outarg{AccessIndex}{idx}{pointer to offsets for each row}
\outarg{AccessIndex}{jdx}{pointer to column indices for each row}
\outarg{AccessIndex}{val}{pointer to values for each row}
\end{fapi}
...
\seealso{ACCESS,SPRS ARRAY ROW DISTRIBUTION,SPRS ARRAY COLUMN DISTRIBUTION}

\apih{SPRS ARRAY ADD ELEMENT}{Add a single element to a sparse array data structure}
...
\begin{funcargs}
\inarg{int}{s_a}{handle for sparse array}
\inarg{int}{idx}{row row index of array element}
\inarg{int}{jdx}{column index of array element}
\inarg{void*}{value}{pointer to element value}
\end{funcargs}
\end{capi}
...
\end{fcode}
\inarg{integer}{s_a}{handle for sparse array}
\inarg{integer}{idx}{row row index of array element}
\inarg{integer}{jdx}{column index of array element}
\inarg{type}{value}{element value}
\end{fapi}
...
\apih{SPRS ARRAY ASSEMBLE}{Organize and distribute elements of sparse array}
...
\begin{figure}
\centering
\includegraphics[width=6.0in,keepaspectratio=true]{sparse-array-layout}
...
\seealso{SPRS ARRAY CREATE}

\apih{SPRS ARRAY COL BLOCK LIST}{Return a list of column blocks with non-zero entries}
...
\begin{funcargs}
\inarg{int}{s_a}{handle for sparse array}
\outarg{int**}{idx}{array of length n containing indices of non-zero blocks}
\outarg{int*}{n}{number of non-zero blocks}
\end{funcargs}
\end{capi}
...
\end{fcode}
\inarg{integer}{s_a}{handle for sparse array}
\outarg{integer}{idx(n)}{array of length n to hold indices}
\inoutarg{integer}{n}{on input: length of idx, on output: number of non-zero
blocks}
\end{fapi}
...
\apih{SPRS ARRAY COLUMN DISTRIBUTION}{Column block associated with each process}
...
\begin{funcargs}
\inarg{int}{s_a}{handle for sparse array}
\inarg{int}{iproc}{process rank}
\outarg{int*}{jlo}{lowest column index associated with process iproc}
\outarg{int*}{jlo}{highest column index associated with process iproc}
\end{funcargs}
\end{capi}
...
\end{fcode}
\inarg{integer}{s_a}{handle for sparse array}
\inarg{integer}{iproc}{process rank}
\outarg{integer}{jlo}{lowest column index associated with process iproc}
\outarg{integer}{jlo}{highest column index associated with process iproc}
\end{fapi}
...
\seealso{SPRS ARRAY ACCESS COL BLOCK}

\apih{SPRS ARRAY CREATE}{Creates a sparse array handle}
\begin{capi}
\begin{ccode}
...
\end{ccode}
\begin{funcargs}
\inarg{int}{idim}{row dimension}
\inarg{int}{jdim}{column dimension}
\inarg{int}{type}{array datatype}
\end{funcargs}
\end{capi}
...
\end{fcode}
\begin{funcargs}
\inarg{integer}{idim}{row dimension}
\inarg{integer}{jdim}{column dimension}
\inarg{integer}{type}{array datatype}
\end{funcargs}
\end{fapi}
...
\seealso{SPRS ARRAY DESTROY}

\apih{SPRS ARRAY CREATE FROM DENSE}{Creates a sparse array from a dense array}
\begin{capi}
\begin{ccode}
...
\end{ccode}
\begin{funcargs}
\inarg{int}{g_a}{handle for dense array}
\end{funcargs}
\end{capi}
...
\end{fcode}
\begin{funcargs}
\inarg{integer}{g_a}{handle for dense array}
\end{funcargs}
\end{fapi}
...
\apih{SPRS ARRAY CREATE FROM SPARSE}{Creates a conventional dense array from a sparse array}
\begin{capi}
\begin{ccode}
...
\begin{fapi}
\begin{fcode}
integer function nga_sprs_array_create_from_sparse(s_a)
\end{fcode}
\begin{funcargs}
\inarg{integer}{s_a}{handle for sparse array}
\end{funcargs}
...
\begin{desc}
This function returns a handle to a conventional dense array formed from a
sparse array. All values in the sparse array are copied to the dense array and
all other values are set to zero.
\end{desc}

\seealso{SPRS ARRAY DESTROY}

\apih{SPRS ARRAY DESTROY}{Destroy an existing sparse array}
...
\apih{SPRS ARRAY DIAG LEFT MULTIPLY}{Left multiply the sparse matrix by a diagonal matrix}
...
\begin{funcargs}
\inarg{int}{s_a}{handle for sparse array}
\inarg{int}{g_d}{handle for global array representing diagonal matrix}
\end{funcargs}
\end{capi}

\begin{fapi}
\begin{fcode}
subroutine nga_sprs_array_diag_left_multiply(s_a, g_d)
\end{fcode}
\inarg{integer}{s_a}{handle for sparse array}
\inarg{integer}{g_d}{handle for global array representing diagonal matrix}
\end{fapi}
...
\apih{SPRS ARRAY DIAG RIGHT MULTIPLY}{Right multiply the sparse matrix by a diagonal matrix}
...
\begin{fapi}
\begin{fcode}
subroutine nga_sprs_array_diag_right_multiply(s_a, g_d)
\end{fcode}
\inarg{integer}{s_a}{handle for sparse array}
\inarg{integer}{g_d}{handle for global array representing diagonal matrix}
\end{fapi}
...
g_d, where g_d is a 1-dimensional vector containing the diagonal values of the
diagonal matrix. The result is stored in s_a.
\end{desc}

\apih{SPRS ARRAY DNSSPRS MULTIPLY}{Multiply a dense matrix by a sparse matrix to get a dense matrix}
...
\begin{funcargs}
\inarg{int}{g_a}{handle for dense array}
\inarg{int}{s_b}{handle for sparse array}
\end{funcargs}
\end{capi}
...
\end{fcode}
\inarg{integer}{g_a}{handle for dense array}
\inarg{integer}{s_b}{handle for sparse array}
\end{fapi}
...
\seealso{SPRS ARRAY SPRSDNS MULTIPLY,SPRS ARRAY MATMAT MULTIPLY}

\apih{SPRS ARRAY DUPLICATE}{Duplicate an existing sparse array}
...
\begin{fapi}
\begin{fcode}
integer function nga_sprs_array_duplicate(s_a)
\end{fcode}
\inarg{integer}{s_a}{handle for sparse array}
\end{fapi}
...
\apih{SPRS ARRAY EXPORT}{Write out non-zero values of a matrix to a file}
...
\begin{funcargs}
\inarg{int}{s_a}{handle for sparse array}
\inarg{char*}{file}{name of file to write out matrix}
\end{funcargs}
\end{capi}
...
\end{fcode}
\inarg{integer}{s_a}{handle for sparse array}
\inarg{character(*)*}{file}{name of file to write out matrix}
\end{fapi}
...
\apih{SPRS ARRAY GET BLOCK}{Retrieve a subblock of the sparse array}
\begin{capi}
\begin{ccode}
...
\end{ccode}
\begin{funcargs}
\inarg{int}{s_a}{sparse array handle}
\inarg{int}{irow}{row index of block}
\inarg{int}{icol}{column index of block}
\outarg{int**}{idx}{array of starting indices for rows in block}
\outarg{int**}{jdx}{array containing column indices of values}
\outarg{void**}{data}{array containing values}
\outarg{int*}{ilo}{lower row index of block}
\outarg{int*}{ihi}{upper row index of block}
\outarg{int*}{jlo}{lower column index of block}
\outarg{int*}{jhi}{upper column index of block}
\end{funcargs}
\end{capi}
...
\apih{SPRS ARRAY GET COLUMN}{Extract the diagonal values of a sparse matrix}
...
\begin{funcargs}
\inarg{int}{s_a}{handle for sparse array}
\inarg{int}{g_d}{index of column to be extracted}
\end{funcargs}
\end{capi}
...
\apih{SPRS ARRAY GET DIAG}{Extract the diagonal values of a sparse matrix}
...
\begin{funcargs}
\inarg{int}{s_a}{handle for sparse array}
\outarg{int*}{g_d}{handle for global array containing diagonal values}
\end{funcargs}
\end{capi}
...
\end{fcode}
\inarg{integer}{s_a}{handle for sparse array}
\outarg{integer}{g_d}{handle for global array containing diagonal values}
\end{fapi}
...
\apih{SPRS ARRAY MATMAT MULTIPLY}{Multiply two sparse matrices to get a third sparse matrix}
...
\apih{SPRS ARRAY MATVEC MULTIPLY}{Multiply a sparse matrix by a vector}
...
\begin{funcargs}
\inarg{int}{s_a}{handle for sparse array}
\inarg{int}{g_a}{handle for vector}
\outarg{int}{g_v}{handle for vector}
\end{funcargs}
\end{capi}
...
\end{fcode}
\inarg{integer}{idim}{row dimension of array}
\inarg{integer}{jdim}{column dimension of array}
\outarg{integer}{type}{array data type}
\end{fapi}
...
\apih{SPRS ARRAY ROW DISTRIBUTION}{Find rows of sparse array owned by process iproc}
\begin{capi}
\begin{ccode}
...
\begin{funcargs}
\inarg{int}{s_a}{sparse array handle}
\inarg{int}{iproc}{processor rank}
\outarg{int*}{lo}{lower index range of rows held by process}
\outarg{int*}{hi}{upper index range of rows held by process}
\end{funcargs}
\end{capi}
...
\end{fcode}
\begin{funcargs}
\inarg{integer}{s_a}{sparse array handle}
\inarg{integer}{iproc}{processor rank}
\outarg{integer}{lo}{lower index range of rows held by process}
\outarg{integer}{hi}{upper index range of rows held by process}
\end{funcargs}
\end{fapi}
...
\apih{SPRS ARRAY SHIFT DIAG}{Shift the diagonal values of the matrix by a constant value}
...
\begin{funcargs}
\inarg{int}{s_a}{handle for sparse array}
\inarg{void*}{val}{amount to shift diagonal}
\end{funcargs}
\end{capi}
...
\end{fcode}
\inarg{int}{s_a}{handle for sparse array}
\inarg{type}{val}{amount to shift diagonal}
\end{fapi}
...
\apih{SPRS ARRAY SPRSDNS MULTIPLY}{Multiply a sparse matrix by a dense matrix to get a dense matrix}
...
\begin{funcargs}
\inarg{int}{s_a}{handle for sparse array}
\inarg{int}{g_b}{handle for dense array}
\end{funcargs}
\end{capi}
...
\end{fcode}
\inarg{integer}{s_a}{handle for sparse array}
\inarg{integer}{g_b}{handle for dense array}
\end{fapi}
...
\seealso{SPRS ARRAY DNSSPRS MULTIPLY,SPRS ARRAY MATMAT MULTIPLY}
...
\apih{UNSET PROPERTY}{Assign a property to a global array}
...
\apih{VERSION}{Returns information on release used to build library}
...
\end{ccode}
\begin{funcargs}
\outarg{int}{major}{major release number}
\outarg{int}{minor}{minor release number}
\outarg{int}{patch}{patch/hotfix release number}
\end{funcargs}
\end{capi}
...
subroutine nga_version(major, minor, patch)
\end{fcode}
\begin{funcargs}
\outarg{integer}{major}{major release number}
\outarg{integer}{minor}{minor release number}
\outarg{integer}{patch}{patch/hotfix release number}
\end{funcargs}
\end{fapi}
